...
export const useToggleLike = ({ paletteId, initialLikes }: UseToggleLikeProps) => {
  ...
  const toggleLike = useCallback(async () => {
    ...
    try {
      ...
      // Update localStorage
      const likedPalettes = JSON.parse(localStorage.getItem("likedPalettes") || "[]");
      const updatedLikes = wasLiked ? likedPalettes.filter((id: string) => id !== paletteId) : [...likedPalettes, paletteId];

      localStorage.setItem("likedPalettes", JSON.stringify(updatedLikes));

      // Only sync with server when liking (not when unliking)
      if (!wasLiked) {
        await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/palettes/${paletteId}/toggleLike`, {
          method: "POST",
        });
      }
    } catch (error) {
